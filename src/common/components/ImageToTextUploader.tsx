...
const ImageToTextUploader = (props: IImageToTextUploader) => {
  ...
  return (
    <div >
      ...
      <Select placeholder="Select Image Language" options={TESSERACT_LANGUAGE_LIST} styles={reactSelectDarkStyle} className='mb-1' onChange={(opt: any) => setLang(opt.value)} />
      <div className="bg-black rounded h-60 lg:h-64">
        {isLoading ? (
          ...
          <label htmlFor="image_input" className="text-white cursor-pointer w-full h-full flex flex-col items-center justify-center">
            <input type="file" name="image_input" id="image_input" hidden size={60} accept="image/png, image/jpeg, image/jpg" onChange={onChange} disabled={false} />
