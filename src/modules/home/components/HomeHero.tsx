  ...
  <div className={classNames("bg-gradient-to-b from-black via-blue-900 to-white h-screen p-4")}>
    ...
    <div className="mt-12 lg:hidden">
      {HEADER_MENU.slice(0, 6).map((menu, i: number) =>
        <Button
          ...
          key={menu.title}
          wrapperClassName={
            classNames("bg-white text-blue-900 rounded-full p-2 w-1/2 mb-4 text-lg font-semibold",
              i % 2 !== 0 ? "ml-auto" : ""
            )}
    ...
    <div className="mt-12 hidden w-1/2 mx-auto lg:grid grid-cols-2 gap-4">
      {HEADER_MENU.slice(0, 6).map((menu) =>
        <div key={menu.title} className="bg-white rounded-md text-black p-2 group">
          <div className="text-2xl flex items-center gap-2 mb-2">
            <span className="text-3xl">{menu.icon}</span>
            <span>{menu.title}</span>
          </div>
          <ul className="list-disc ml-4">
            {menu.desc?.map((des) => <li key={des}>
              {des}
            </li>)}
          </ul>
