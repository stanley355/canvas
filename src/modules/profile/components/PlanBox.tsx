...
const PlanBox = (props: IPlanBox) => {
  ...
  const subscriptionExpired = subscription?.id ? isSubscriptionExpired(subscription.end_at) : null;
    ...
    <div className='border border-gray-500 rounded p-2 my-4 lg:my-0 lg:w-2/3 '>
      ...
      {(!subscription?.id && !user.balance) &&
        <div className='flex flex-col items-center my-8 lg:text-xl w-fit shadow-lg p-4 mx-auto rounded'>
          ...
          <div className='text-xl  lg:text-2xl font-semibold'>Premium Plan Not Found </div>
      ...
      {(subscription?.id && !subscriptionExpired) &&
        <div className='flex flex-col items-center my-8 lg:text-xl w-fit shadow-lg p-4 mx-auto rounded'>
          ...
          <div className='text-xl  lg:text-2xl font-semibold'>{formatSubscriptionMonth(subscription.duration_type)} Subscription </div>
          <div>Use Premium all the way until</div>
          <div className='font-semibold text-lg lg:text-xl mt-2'> {formatSubscriptionEndDate(subscription.end_at)}</div>
