...
const SpeechToTextProvider = ({ children }: SpeechToTextProviderProps) => {
  const [speechToTextStates, speechToTextDispatch] = useReducer(speechToTextReducer, SPEECH_TO_TEXT_STATES);

  return (
    <SpeechToTextContext.Provider value={{ speechToTextStates, speechToTextDispatch }}>
      {children}
