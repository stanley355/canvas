...
const TranslateLanguageOptionsDesktop = (
  props: ITranslateLanguageOptionsDesktop
) => {
  ...
  const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.value) {
      const newList = TRANSLATE_LANGUAGE_LIST_V2.filter(
        (lang: { label: string; value: string }) =>
          lang.label.includes(e.target.value)
      );
      setLangList(newList);
      return;
    }
  ...
  return (
    <div className="absolute left-0 z-40 w-full overflow-hidden bg-white border rounded-lg top-10 h-72">
      ...
      <div className="grid grid-cols-6">
        ...
        {langList.map((language: { label: string; value: string }) => (
          <Button
            ...
            className={cn(
              isSource && sourceLanguage.label === language.label
                ? "bg-blue-100 text-blue-800"
                : "",
              !isSource && targetLanguage.label === language.label
                ? "bg-blue-100 text-blue-800"
                : ""
            )}
          >
