...
const TranslateLanguageOptionsMobile = (
  ...
  const handleSearch = (text: string) => {
    if (text) {
      const newList = TRANSLATE_LANGUAGE_LIST_V2.filter((option: IOption) => option.label.toLowerCase().includes(text.toLowerCase()));
  ...
  const handleDetectLanguage = () => {
    dispatch({ type: "SET", name: "sourceLanguage", value: { label: "Detect Language", value: "" } });
    onCloseClick();
    return;
  };

  const handleOptionClick = (option: IOption) => {
    dispatch({ type: "SET", name: isSource ? "sourceLanguage" : "targetLanguage", value: option });
    onCloseClick();
    return;
  }
